...
const mockTutorClasses = [
  {
    ...
    studentsPresent: 43,
    materials: [
       { id: 1, name: "De_cuong_mon_hoc.pdf", size: "1.5 MB", type: "pdf", date: "2025-09-05" },
       { id: 2, name: "Bai_giang_Chuong_1.pptx", size: "5.2 MB", type: "ppt", date: "2025-09-10" }
    ],
  ...
  },
  {
    ...
    studentsPresent: 55,
    materials: [
       { id: 1, name: "Assignment_1.pdf", size: "500 KB", type: "pdf", date: "2025-10-01" }
    ],
...
function renderTutorCourses() {
    const container = document.getElementById("tutor-courses-list");
    if (!container) return;

    container.innerHTML = mockTutorClasses.map(c => {
        ...
        const avgScore = c.students.reduce((acc, s) => acc + (parseFloat(s.average) || 0), 0) / (c.students.filter(s => s.average !== "--").length || 1);
        ...
        <div class="glass-card p-6 rounded-[28px] group hover:border-blue-300 transition-all duration-300 relative overflow-hidden">
            ...
            <div class="flex flex-col md:flex-row gap-6 items-start">
                ...
                <div class="flex-1 w-full min-w-0">
                    <div class="flex justify-between items-start">
                        <div class="min-w-0">
                            <span class="text-[10px] font-bold bg-blue-50 text-blue-600 px-2 py-1 rounded uppercase tracking-wider shadow-sm">${c.id}</span>
                            <h4 class="text-xl font-black text-slate-800 mt-2 truncate">${c.name} - ${c.group}</h4>
                            <p class="text-sm text-slate-500 mt-1 truncate font-medium">
                                <i class="fa-solid fa-location-dot mr-1.5 text-red-400"></i>Phòng: ${c.room}
                                <span class="mx-2">•</span>
                                <i class="fa-solid fa-clock mr-1.5 text-orange-400"></i>${c.schedule}
                            </p>
                        </div>
                        ${statusBadge}
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-4 mt-5 max-w-md">
                        <div class="bg-slate-50/80 p-2.5 rounded-xl border border-slate-100 text-center hover:bg-white hover:shadow-sm transition">
                            ...
                            <p class="font-black text-slate-700 text-lg">${c.studentsCount}</p>
                        </div>
                        <div class="bg-slate-50/80 p-2.5 rounded-xl border border-slate-100 text-center hover:bg-white hover:shadow-sm transition">
                            <p class="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Vắng</p>
                            <p class="font-black text-red-500 text-lg">${c.studentsCount - c.studentsPresent}</p>
                        </div>
                        <div class="bg-slate-50/80 p-2.5 rounded-xl border border-slate-100 text-center hover:bg-white hover:shadow-sm transition">
                            <p class="text-[10px] text-slate-400 uppercase font-bold tracking-wider">ĐTB</p>
                            <p class="font-black text-emerald-500 text-lg">${isNaN(avgScore) ? "--" : avgScore.toFixed(1)}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-3 w-full md:w-auto flex-shrink-0 mt-4 md:mt-0">
                    <button onclick="toggleTutorClassDetail('${c.id}')" class="px-5 py-3 rounded-xl bg-blue-600 text-white text-xs font-bold shadow-lg shadow-blue-500/30 hover:bg-blue-700 transition flex items-center justify-center gap-2 w-full md:w-auto">
                        <i class="fa-solid fa-folder-open"></i> Quản lý & Chi tiết
                    </button>
                    <div class="flex gap-2">
                         <button class="flex-1 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 transition flex items-center justify-center gap-2">
                            <i class="fa-solid fa-list-check"></i> Điểm danh
                        </button>
                        <button class="px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-xs font-bold text-slate-600 hover:text-blue-600 hover:border-blue-200 transition">
            ...
            <!-- Expandable Details -->
            <div id="details-${c.id}" class="hidden mt-6 pt-6 border-t border-slate-100 animate-fade-in-down">
                <!-- Tabs for Details -->
                <div class="flex border-b border-slate-200 mb-6 overflow-x-auto">
                    <button class="detail-tab-btn active px-4 py-2 text-sm font-bold text-blue-600 border-b-2 border-blue-600 transition whitespace-nowrap" onclick="switchDetailTab(this, 'tab-students-${c.id}')">
                        <i class="fa-solid fa-user-graduate mr-2"></i>Sinh viên & Điểm
                    </button>
                    <button class="detail-tab-btn px-4 py-2 text-sm font-bold text-slate-500 hover:text-blue-600 transition whitespace-nowrap" onclick="switchDetailTab(this, 'tab-materials-${c.id}')">
                        <i class="fa-solid fa-folder-open mr-2"></i>Tài liệu
                    </button>
                    <button class="detail-tab-btn px-4 py-2 text-sm font-bold text-slate-500 hover:text-blue-600 transition whitespace-nowrap" onclick="switchDetailTab(this, 'tab-stats-${c.id}')">
                ...
                <!-- 1. Student List & Progress (Merged) -->
                <div id="tab-students-${c.id}" class="detail-tab-content">
                     <div class="flex justify-between items-center mb-4">
                        ...
                        <button onclick="exportClassData('${c.id}')" class="px-3 py-1.5 rounded-lg bg-green-100 text-green-700 text-xs font-bold hover:bg-green-200 transition">
                            ...
                            <tbody class="text-sm text-slate-700 divide-y divide-slate-100 bg-white">
                                ...
                                <tr class="hover:bg-blue-50/50 transition">
                                    ...
                                    <td class="p-3 text-center">
                                        <div class="w-full bg-slate-200 rounded-full h-1.5 mb-1 overflow-hidden">
                                            <div class="bg-blue-500 h-1.5 rounded-full" style="width: ${s.attendance}%"></div>
                                        </div>
                                        <span class="text-[10px] text-slate-500">${s.attendance}%</span>
                                    </td>
                                    <td class="p-3 text-center font-mono">${s.midterm}</td>
                                    <td class="p-3 text-center font-mono">${s.final}</td>
                                    <td class="p-3 text-center font-bold font-mono text-blue-600">${s.average}</td>
                                ...
                                <tr>
                                    <td colspan="7" class="p-3 text-center text-xs text-slate-400 italic bg-slate-50">
                                        ... và ${c.students.length - 5} sinh viên khác ...
                                    </td>
                                </tr>` : ''}
                            </tbody>
                        </table>
                ...
                <div id="tab-materials-${c.id}" class="detail-tab-content hidden">
                    <div class="flex justify-between items-center mb-4">
                        <h5 class="text-sm font-bold text-slate-700">Tài liệu đã tải lên (${c.materials.length})</h5>
                        <button onclick="openUploadModal('${c.id}')" class="px-3 py-2 rounded-xl bg-blue-600 text-white text-xs font-bold hover:bg-blue-700 transition shadow-md">
                            <i class="fa-solid fa-cloud-arrow-up mr-2"></i>Upload mới
                        </button>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ${c.materials.length > 0 ? c.materials.map(m => `
                        <div class="flex items-center justify-between p-3 border border-slate-200 rounded-xl bg-white hover:border-blue-400 transition group/file">
                            <div class="flex items-center gap-3 overflow-hidden">
                                <div class="w-10 h-10 bg-${m.type === 'pdf' ? 'red' : 'blue'}-50 text-${m.type === 'pdf' ? 'red' : 'blue'}-500 rounded-lg flex items-center justify-center text-lg flex-shrink-0">
                                    <i class="fa-solid fa-file-${m.type === 'pdf' ? 'pdf' : 'powerpoint'}"></i>
                                </div>
                                <div class="min-w-0">
                                    <p class="text-xs font-bold text-slate-700 truncate">${m.name}</p>
                                    <p class="text-[10px] text-slate-500">${m.size} • ${m.date}</p>
                                </div>
                            </div>
                        ...
                        `).join("") : `<p class="text-slate-400 text-sm col-span-2 text-center py-4">Chưa có tài liệu nào.</p>`}
                    </div>
                </div>
                        ...
                        <div class="p-4 border border-slate-200 rounded-xl bg-white">
                            ...
                            <ul class="space-y-3 text-sm">
                                <li class="flex justify-between">
                                    ...
                                    <span class="font-bold text-slate-800">${avgScore.toFixed(2)}</span>
                                </li>
                                <li class="flex justify-between">
...
function toggleTutorClassDetail(id) {
...
function switchDetailTab(btn, targetId) {
    // Find parent container to limit scope
...
function submitUpload() {
    const btn = document.querySelector('#upload-modal button[onclick="submitUpload()"]');
    ...
    setTimeout(() => {
        ...
        closeUploadModal();
        // In a real app, we would refresh the materials list here
    }, 1000);
}

function exportClassData(courseId) {
